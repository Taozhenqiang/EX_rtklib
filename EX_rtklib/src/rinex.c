                        ...
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
    ...
    trace(4,"decode_obsh: ver=%.2f label=%s",ver,label);//label后面本身包含换行符
    ...
    if      (strstr(label,"MARKER NAME"         )) {
        ...
        for (j=nt=0,k=7;j<n;j++,k+=4) {
			...
			trace(2, "sys=%c i=%d j=%2d tobs[i][j]=%s\n", sys, i,nt-1, tobs[i][nt-1]);//测试
        }
        *tobs[i][nt]='\0';
        ...
        for (i=nt=0,j=10;i<n;i++,j+=6) {
            ...
            if (ver<=2.99) {
                ...
                convcode(ver,SYS_CMP,str,tobs[5][nt]);//将观测值类型转化到所有系统
            }
            nt++;
        }
        *tobs[0][nt]='\0';
...
static void decode_navh(char *buff, nav_t *nav)
{
    ...
    trace(4,"decode_navh: label=%s",label);
    ...
    if      (strstr(label,"ION ALPHA"           )) { /* opt ver.2 */
        ...
        if (nav) {
            if (!strncmp(buff,"GPSA",4)) {
				...
				if (isalpha(buff[54])) {
					int k = toupper(buff[54]) - 65;
					for (i=0,j=5;i<4;i++,j+=12) nav->ion_bds[k][i]=str2num(buff,j,12);
				}
				...
				if (isalpha(buff[54])) {
					int k = toupper(buff[54]) - 65;
					for (i=0,j=5;i<4;i++,j+=12) nav->ion_bds[k][i+4]=str2num(buff,j,12);
				}
                           ...
                           int *flag, int *sats)
{
    ...
    trace(2,"decode_obsepoch: time=%s flag=%d\n",time_str(*time,3),*flag);
    return n;
}
                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    //trace(4,"decode_obsdata: ver=%.2f sat=%3d id=%-3s\n",ver, obs->sat,id);
    
    if (ver>2.99) { /* ver.3 */
        sprintf(satid,"%.3s",buff);
        obs->sat=(uint8_t)satid2no(satid);
    }
	satno2id(obs->sat, id);//测试
    if (!obs->sat) {
    ...
    for (i=0;i<ind->n;i++) {
		if (p[i] < 0 || val[i] == 0.0) {
			trace(4, "sys=%2d sat=%3d id=%-3s obscodes=%s ind->type=%d code=%s\n",
				satsys(obs->sat, NULL), obs->sat,id, obscodes, ind->type[i],
				code2obs(ind->code[i])); continue;}
        switch (ind->type[i]) {
            case 0: obs->P[p[i]]=val[i]; obs->code[p[i]]=ind->code[i]; 
				trace(4, "sys=%2d sat=%3d id=%-3s obscodes=%s ind->type=%d code=%s p[i]=%2d obs->P[p[i]]=%13.3f obs->code[p[i]]=%2d\n", 
					satsys(obs->sat, NULL), obs->sat,id,obscodes, ind->type[i], 
					code2obs(ind->code[i]), p[i],val[i],ind->code[i]); break;//测试
            case 1: obs->L[p[i]]=val[i]; obs->LLI [p[i]]=lli[i]; obs->code[p[i]]=ind->code[i]; 
				trace(4, "sys=%2d sat=%3d id=%-3s obscodes=%s ind->type=%d code=%s p[i]=%2d obs->L[p[i]]=%13.3f obs->LLI[p[i]]=%hhu\n",
					satsys(obs->sat, NULL), obs->sat,id, obscodes, ind->type[i], 
					code2obs(ind->code[i]), p[i],val[i],lli[i]); break;
            case 2: obs->D[p[i]]=(float)val[i];                     
				trace(4, "sys=%2d sat=%3d id=%-3s obscodes=%s ind->type=%d code=%s p[i]=%2d obs->D[p[i]]=%13.3f\n",
					satsys(obs->sat, NULL), obs->sat, id,obscodes, ind->type[i], 
					code2obs(ind->code[i]),p[i],val[i]); break;
            case 3: obs->SNR[p[i]]=(uint16_t)(val[i]/SNR_UNIT+0.5);
				trace(4, "sys=%2d sat=%3d id=%-3s obscodes=%s ind->type=%d code=%s p[i]=%2d obs->SNR[p[i]]=%3.1f\n",
					satsys(obs->sat, NULL), obs->sat,id, obscodes, ind->type[i],
					code2obs(ind->code[i]), p[i],obs->SNR[p[i]]*SNR_UNIT);break;
        }
    }
                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    for (i=n=0;*tobs[i];i++,n++) {
        ind->code[i]=obs2code(tobs[i]+1);//列地址
        ind->type[i]=(p=strchr(obscodes,tobs[i][0]))?(int)(p-obscodes):0;
        ind->idx[i]=code2idx(sys,ind->code[i]);//frequency index
...
//#if 0 /* for debug */
	if (32 == sys) {
		for (i = 0; i < n; i++) {
			trace(2, "sys=%3d i=%2d code=%2s ind->code[i]=%2d obscodes=%s ind->type[i]=%d ind->idx[i]=%2d ind->pri[i]=%2d ind->pos[i]=%2d\n",
				sys, i, code2obs(ind->code[i]), ind->code[i], obscodes, ind->type[i], ind->idx[i], ind->pri[i],
				ind->pos[i]);//测试
		}
                      ...
                      eph_t *eph)
{
    ...
    trace(4,"decode_eph: ver=%.2f id=%-3s sat=%2d\n",ver,id,sat);
    ...
    *eph=eph0;
	...
	eph->DataFrom=(int)data[20];
	if (sys == SYS_CMP && eph->DataFrom&7)
	{
		eph->delA=data[10]; eph->e=data[ 8];   eph->i0  =data[15]; eph->OMG0=data[13];
        ...
        eph->cus =data[ 9]; eph->cic=data[12]; eph->cis =data[14]; eph->Adot=data[22];
		eph->delndot=data[29]; eph->SatType=(int)data[30];
	}
	else{
    ...
    if (sys==SYS_GPS||sys==SYS_QZS) {
		...
		{
			eph->acc = data[23];
			eph->tgd[2]=   data[25];      /* TGD1 B1C/B3 */
			eph->tgd[4]=   data[26];      /* ISC_B1Cd */
		}
		else if (eph->DataFrom == 4)
		{
			eph->acc = data[23];
			eph->tgd[3]=   data[25];      /* TGD1 B1C/B3 */
			eph->tgd[5]=   data[26];      /* ISC_B2ad */
		}
		else
		{
			eph->sva = uraindex(data[23]);  /* URA index (m->index) */
  ...
  //      if (data[20] == 1)  eph->tgd[2]=   data[25]; /* TGD1 B1C/B3 */
		//else if (data[20] == 4) eph->tgd[3]=   data[25]; /* TGD1 B2a/B3 */
		//else {
  //      eph->tgd[0]=   data[25];      /* TGD1 B1/B3 */
  //      eph->tgd[1]=   data[26];      /* TGD2 B2/B3 */
		//}
