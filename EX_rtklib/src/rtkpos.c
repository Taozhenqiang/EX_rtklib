    ...
    if (est) {
		...
		if(rtk->sol.stat == 2){
		   /*p+=sprintf(p,"$POS,%d,%.3f,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n",week,tow,
				   rtk->sol.stat,rtk->x[0],rtk->x[1],rtk->x[2],0,0,0);*/
		   p+=sprintf(p,"$POS,%d,%.3f,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.1f,%d,%d,%d,%d,%d\n",week,tow,
                   rtk->sol.stat,rtk->x[0],rtk->x[1],rtk->x[2],0,0,0,rtk->sol.dop[1],rtk->sol.ratio,
		           rtk->sol.satnum[0],rtk->sol.satnum[1],rtk->sol.satnum[2],rtk->sol.satnum[3],rtk->sol.satnum[4]);
		}
		else {
		   p+=sprintf(p,"$POS,%d,%.3f,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.1f,%d,%d,%d,%d,%d\n",week,tow,
                   rtk->sol.stat,rtk->x[0],rtk->x[1],rtk->x[2],xa[0],xa[1],xa[2],rtk->sol.dop[1],rtk->sol.ratio,
			       rtk->sol.satnum[0],rtk->sol.satnum[1],rtk->sol.satnum[2],rtk->sol.satnum[3],rtk->sol.satnum[4]);
		}         
    }
    else {
    ...
    for (i=0;i<MAXSAT;i++) {
        ...
        for (j=0;j<nfreq;j++) {
            fprintf(fp_stat,"$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.1f,%d,%d,%d,%d,%d,%d\n",
                    week,tow,id,p[j]+1,ssat->azel[0]*R2D,ssat->azel[1]*R2D,
                    ssat->resp[p[j]],ssat->resc[p[j]],ssat->vsat[p[j]],
                    ssat->snr[p[j]]*SNR_UNIT,ssat->fix[p[j]],ssat->slip[p[j]]&3,
                    ssat->lock[p[j]],ssat->outc[p[j]],ssat->slipc[p[j]],ssat->rejc[p[j]]);
			/*trace(1,"$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.1f,%d,%d,%d,%d,%d,%d\n",
                    week,tow,id,p[j]+1,ssat->azel[0]*R2D,ssat->azel[1]*R2D,
                    ssat->resp[p[j]],ssat->resc[p[j]],ssat->vsat[p[j]],
                    ssat->snr[p[j]]*SNR_UNIT,ssat->fix[p[j]],ssat->slip[p[j]]&3,
                    ssat->lock[p[j]],ssat->outc[p[j]],ssat->slipc[p[j]],ssat->rejc[p[j]]);*/
...
static double gfobs(rtk_t *rtk,const obsd_t *obs, int i, int j, int k, const nav_t *nav)
{
    double freq1,freq2,L1,L2;
	int *p = rtk->opt.fre;
    
	if (satsys(obs[i].sat,NULL) == SYS_CMP && rtk->opt.ionoopt==IONOOPT_IFLC){
	    freq1=sat2freq(obs[i].sat,obs[i].code[0],nav);
		freq2=sat2freq(obs[i].sat,obs[i].code[3],nav);
		L1=sdobs(obs,i,j,0);
        L2=sdobs(obs,i,j,3);
	}		   
	else {
		freq1=sat2freq(obs[i].sat,obs[i].code[p[0]],nav);
		freq2=sat2freq(obs[i].sat,obs[i].code[p[k]],nav);
        L1=sdobs(obs,i,j,p[0]);
        L2=sdobs(obs,i,j,p[k]);
	}
                  ...
                  const prcopt_t *opt, int *sat, int *iu, int *ir)
{
    ...
    for (i=0,j=nu;i<nu&&j<nu+nr;i++,j++) {
        if      (obs[i].sat<obs[j].sat) j--;
            ...
            trace(3,"(%2d) sat=%3d id=%-3s iu=%2d ir=%2d\n",
				k-1,obs[i].sat,id,i,j);//≤‚ ‘
        }
    }
	trace(3, "common satellites number=%2d\n",k);//≤‚ ‘
    return k;
}
...
static void udpos(rtk_t *rtk, double tt)
{
	...
	trace(4, "P=\n"); tracemat(4, P, nx, nx, 13, 6);//≤‚ ‘
	trace(4, "time update: xp=\n"); tracemat(4, xp, nx, 1, 13, 6);//≤‚ ‘
	...
	trace(4, "rtk->P=\n"); tracemat(4, rtk->P, rtk->nx, rtk->nx, 13, 6);//≤‚ ‘
    /* process noise added to only acceleration */
    Q[0]=Q[4]=SQR(rtk->opt.prn[3])*fabs(tt);
    Q[8]=SQR(rtk->opt.prn[4])*fabs(tt);
	trace(4, "Q=\n"); tracemat(4, Q, 3, 3, 13, 6);//≤‚ ‘
    ecef2pos(rtk->x,pos);
    covecef(pos,Q,Qv);//transform local enu covariance to xyz-ecef coordinate
	trace(4, "Qv=\n"); tracemat(4, Qv, 3, 3, 13, 6);//≤‚ ‘
...
static void detslp_ll(rtk_t *rtk, const obsd_t *obs, int i, int rcv)
{
    ...
    for (f=0;f<rtk->opt.nf;f++) {
		if (1 == f && satsys(obs[i].sat, NULL) == SYS_CMP && rtk->opt.ionoopt == IONOOPT_IFLC) {
			if (obs[i].L[3] == 0.0 ||
				fabs(timediff(obs[i].time,rtk->ssat[sat-1].pt[rcv-1][3])) < DTTOL) {
				continue;
			}
			/* restore previous LLI */
			if (rcv==1) LLI = getbitu(&rtk->ssat[sat-1].slip[3], 0, 2); /* rover */
			else        LLI = getbitu(&rtk->ssat[sat-1].slip[3], 2, 2); /* base  */

			/* detect slip by cycle slip flag in LLI */
			if (rtk->tt >= 0.0) { /* forward */
				if (obs[i].LLI[3] & 1) {
					trace(4, "LLi=%hhu", obs[i].LLI[3]);
					errmsg(rtk, "slip detected forward  (sat=%2d rcv=%d F=%d LLI=%x)\n",
						sat, rcv, f + 1, obs[i].LLI[3]);
				}
				slip = obs[i].LLI[3];
			}
			else { /* backward */
			...
			if (((LLI & 2) && !(obs[i].LLI[3] & 2)) || (!(LLI & 2) && (obs[i].LLI[3] & 2))) {
				errmsg(rtk, "slip detected half-cyc (sat=%2d rcv=%d F=%d LLI=%x->%x)\n",
					sat,rcv,f+1,LLI,obs[i].LLI[f]);
				slip |= 1;
			}
			/* save current LLI */
			if (rcv==1) setbitu(&rtk->ssat[sat-1].slip[3], 0, 2, obs[i].LLI[f]);
			else        setbitu(&rtk->ssat[sat-1].slip[3], 2, 2, obs[i].LLI[f]);

			/* save slip and half-cycle valid flag */
			rtk->ssat[sat-1].slip[3] |= (uint8_t)slip;
			rtk->ssat[sat-1].half[3]  = (obs[i].LLI[f] & 2) ? 0 : 1;
			trace(3, " idx=%3d f=%d rtk->ssat[sat-1].slip[3]=%d rtk->ssat[sat-1].half[3]=%d\n",
				sat-1, f, rtk->ssat[sat-1].slip[3], rtk->ssat[sat-1].half[3]);//≤‚ ‘£¨ÃΩ≤‚µΩ∞Î÷‹Ã¯half÷√1
		}
		else {
			if (obs[i].L[p[f]] == 0.0 ||
				fabs(timediff(obs[i].time, rtk->ssat[sat-1].pt[rcv-1][p[f]])) < DTTOL) {
				continue;
			}
			/* restore previous LLI */
			if (rcv==1) LLI = getbitu(&rtk->ssat[sat-1].slip[p[f]], 0, 2); /* rover */
			else        LLI = getbitu(&rtk->ssat[sat-1].slip[p[f]], 2, 2); /* base  */

			/* detect slip by cycle slip flag in LLI */
			if (rtk->tt >= 0.0) { /* forward */
				if (obs[i].LLI[p[f]] & 1) {
					trace(4, "LLi=%hhu", obs[i].LLI[p[f]]);
					errmsg(rtk, "slip detected forward  (sat=%2d rcv=%d F=%d LLI=%x)\n",
						sat,rcv,f+1,obs[i].LLI[p[f]]);
				}
				slip = obs[i].LLI[p[f]];
			}
			else { /* backward */
			...
			/* detect slip by parity unknown flag transition in LLI */
			if (((LLI&2) && !(obs[i].LLI[p[f]]&2)) || (!(LLI&2) && (obs[i].LLI[p[f]]&2))) {
				errmsg(rtk, "slip detected half-cyc (sat=%2d rcv=%d F=%d LLI=%x->%x)\n",
					sat,rcv,f+1,LLI,obs[i].LLI[p[f]]);
				slip|=1;
			}
			/* save current LLI */
			if (rcv==1) setbitu(&rtk->ssat[sat-1].slip[p[f]], 0, 2, obs[i].LLI[p[f]]);
			else        setbitu(&rtk->ssat[sat-1].slip[p[f]], 2, 2, obs[i].LLI[p[f]]);

			/* save slip and half-cycle valid flag */
			rtk->ssat[sat-1].slip[p[f]] |= (uint8_t)slip;
			rtk->ssat[sat-1].half[p[f]]  = (obs[i].LLI[p[f]]&2) ? 0 : 1;
			/*trace(1, " idx=%3d f=%d rtk->ssat[sat-1].slip[f]=%d rtk->ssat[sat-1].half[f]=%d\n",
				sat-1, p[f], rtk->ssat[sat-1].slip[p[f]], rtk->ssat[sat-1].half[p[f]]);*///≤‚ ‘£¨ÃΩ≤‚µΩ∞Î÷‹Ã¯half÷√1
		}
    }
}
                      ...
                      const nav_t *nav)
{
    ...
    for (k=1;k<rtk->opt.nf;k++) {
        if ((g1=gfobs(rtk,obs,i,j,k,nav))==0.0) return;//≤‚ ‘
         
        g0=rtk->ssat[sat-1].gf[p[k]-1];
        rtk->ssat[sat-1].gf[p[k]-1]=g1;
         
        if (g0!=0.0&&fabs(g1-g0)>rtk->opt.thresslip) {
                rtk->ssat[sat-1].slip[p[0]]|=1;
			if (satsys(sat, NULL) == SYS_CMP && rtk->opt.ionoopt == IONOOPT_IFLC) {
				rtk->ssat[sat-1].slip[3]|=1;//≤‚ ‘
				//trace(1,"slip detected GF jump (sat=%2d id=%-3s L1-L%d GF=%.3f %.3f)  time=%s\n",sat,id,4,g0,g1,time_str(obs[0].time,3));
				errmsg(rtk,"slip detected GF jump (sat=%2d L1-L%d GF=%.3f %.3f)\n",
                   sat,4,g0,g1);
			}
			else {
				rtk->ssat[sat-1].slip[p[k]]|=1;
				//trace(1,"slip detected GF jump (sat=%2d id=%-3s L1-L%d GF=%.3f %.3f)  time=%s\n",sat,id,k+1,g0,g1,time_str(obs[0].time,3));
				errmsg(rtk,"slip detected GF jump (sat=%2d L1-L%d GF=%.3f %.3f)\n",
                   sat,k+1,g0,g1);
			}
        }
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    for (i=0;i<ns;i++) {
		...
		for (k = 0; k < rtk->opt.nf; k++) {
			if ( 1 == k && satsys(obs[iu[i]].sat, NULL) == SYS_CMP && rtk->opt.ionoopt == IONOOPT_IFLC)
			     rtk->ssat[sat[i]-1].slip[3]&=0xFC;
			else rtk->ssat[sat[i]-1].slip[p[k]]&=0xFC;
		}			
        ...
        for (k=0;k<nf;k++) {
            rtk->ssat[sat[i]-1].half[p[k]]=
                !((obs[iu[i]].LLI[p[k]]&2)||(obs[ir[i]].LLI[p[k]]&2));
			trace(3, " sat[i-1]=%3d k=%d rtk->ssat[sat[i]-1].half[k]=%d\n", 
				sat[i]-1,p[k], rtk->ssat[sat[i]-1].half[p[k]]);//≤‚ ‘
        }
    }
    for (k=0;k<nf;k++) {
        /* reset phase-bias if instantaneous AR or expire( ß–ß) obs outage counter */
        for (i=1;i<=MAXSAT;i++) {
            
            reset=++rtk->ssat[i-1].outc[p[k]]>(uint32_t)rtk->opt.maxout;
            ...
            if (rtk->opt.modear==ARMODE_INST&&rtk->x[IB(i,k,&rtk->opt)]!=0.0) {
                ...
                trace(3,"udbias : obs outage counter overflow (sat=%3d L%d n=%d)\n",
                      i,p[k]+1,rtk->ssat[i-1].outc[p[k]]);
                rtk->ssat[i-1].outc[p[k]]=0;
				if (satsys(i,NULL) == SYS_CMP){
				    satno2id(i, id);//≤‚ ‘
				    /*trace(1, "time=%s fre=%d id=%-3s outc=%4d\n",
						 time_str(obs[0].time,3),k,id,rtk->ssat[i-1].outc[k]); */ //≤‚ ‘
				}
            }
            if (rtk->opt.modear!=ARMODE_INST&&reset) {
                rtk->ssat[i-1].lock[p[k]]=-rtk->opt.minlock;//◊¢“‚ «∏≥÷µ≤Ÿ◊˜≤ª «ºı∑®
				if (satsys(i,NULL) == SYS_CMP){
				    satno2id(i, id);//≤‚ ‘
				    /*trace(1, "time=%s fre=%d id=%-3s lock=%4d reset=%d\n",
					    time_str(obs[0].time,3),k,id,rtk->ssat[i-1].lock[k],reset);*/  //≤‚ ‘
				}
            }
        }
        ...
        for (i=0;i<ns;i++) {
            ...
            slip=rtk->ssat[sat[i]-1].slip[p[k]];
            if (rtk->opt.ionoopt==IONOOPT_IFLC) {
				if (satsys(sat[i],NULL) == SYS_CMP)
					 slip|=rtk->ssat[sat[i]-1].slip[3];//≤‚ ‘
			    else slip|=rtk->ssat[sat[i]-1].slip[1];
			}
            if (rtk->opt.modear==ARMODE_INST||!(slip&1)) continue;//ÃΩ≤‚µΩ÷‹Ã¯ ±slip÷√1
            rtk->x[j]=0.0;
            rtk->ssat[sat[i]-1].lock[p[k]]=-rtk->opt.minlock;
			if (satsys(sat[i],NULL) == SYS_CMP){
				satno2id(sat[i], id);//≤‚ ‘
				//trace(1, "time=%s fre=%d id=%-3s lock=%4d slip=%2d %2d\n",
				//	  time_str(obs[0].time,3),p[k],id,rtk->ssat[sat[i]-1].lock[p[k]],rtk->ssat[sat[i]-1].slip[0],rtk->ssat[sat[i]-1].slip[3]);  //≤‚ ‘
			}
        }
        bias=zeros(ns,1);
        ...
        for (i=j=0,offset=0.0;i<ns;i++) {
            ...
            if (rtk->opt.ionoopt!=IONOOPT_IFLC) {
                cp=sdobs(obs,iu[i],ir[i],p[k]); /* cycle */
                pr=sdobs(obs,iu[i],ir[i],p[k]+NFREQ);
                freqi=sat2freq(sat[i],obs[iu[i]].code[p[k]],nav);
				...
				if (satsys(sat[i], NULL) == SYS_CMP) {
                   ...
                   cp2=sdobs(obs,iu[i],ir[i],3);//≤‚ ‘B1I+B3I
				   pr1=sdobs(obs,iu[i],ir[i],NFREQ);
				   pr2=sdobs(obs,iu[i],ir[i],NFREQ+3);//≤‚ ‘B1I+B3I
				   freq1=sat2freq(sat[i],obs[iu[i]].code[0],nav);
                   freq2=sat2freq(sat[i],obs[iu[i]].code[3],nav);
				}
				else {
				   cp1=sdobs(obs,iu[i],ir[i],0);				
        ...
        /* correct phase-bias offset to enssure phase-code coherency(“ª÷¬–‘) */
        if (j>0) {
            for (i=1;i<=MAXSAT;i++) {
        ...
        for (i=0;i<ns;i++) {
			if (bias[i] == 0.0||rtk->x[IB(sat[i],k,&rtk->opt)] != 0.0) {
				trace(3, "i=%2d idx=%3d bias=%10.6f rtk->x=%.6f\n", 
					i,IB(sat[i],k,&rtk->opt),bias[i],rtk->x[IB(sat[i],k,&rtk->opt)]);//≤‚ ‘
				continue;
			}
            initx(rtk,bias[i],SQR(rtk->opt.std[0]),IB(sat[i],k,&rtk->opt));
			trace(3, "i=%2d idx=%3d rtk->x=%.6f\n",i,IB(sat[i], k, &rtk->opt), bias[i]);//≤‚ ‘
        }
		free(bias);
    }
}
                      ...
                      const prcopt_t *opt, double *y, double *freq)
{
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* iono-free linear combination */
		...
		if (satsys(obs->sat,NULL) == SYS_CMP)
			freq2=sat2freq(obs->sat,obs->code[3],nav);//≤‚ ‘
        else freq2=sat2freq(obs->sat,obs->code[1],nav);
      ...
      //    testsnr(base,1,azel[1],obs->SNR[1]*SNR_UNIT,&opt->snrmask)) return;
		if (testsnr(base,0,azel[1],obs->SNR[0]*SNR_UNIT,&opt->snrmask)|| 
			 (satsys(obs->sat,NULL) == SYS_CMP ? testsnr(base,3,azel[1],obs->SNR[3]*SNR_UNIT,&opt->snrmask) : testsnr(base,1,azel[1],obs->SNR[1]*SNR_UNIT,&opt->snrmask))) return;
        
        C1= SQR(freq1)/(SQR(freq1)-SQR(freq2));
        C2=-SQR(freq2)/(SQR(freq1)-SQR(freq2));
        dant_if=C1*dant[0]+C2*dant[1];
        
        if ((obs->L[0]!=0.0&&obs->L[3]!=0.0) || (obs->L[0]!=0.0&&obs->L[1]!=0.0)) {
			if (satsys(obs->sat,NULL) == SYS_CMP)
				y[0]=C1*obs->L[0]*CLIGHT/freq1+C2*obs->L[3]*CLIGHT/freq2-r-dant_if;//≤‚ ‘
            else y[0]=C1*obs->L[0]*CLIGHT/freq1+C2*obs->L[1]*CLIGHT/freq2-r-dant_if;
        }
        if ((obs->P[0]!=0.0&&obs->P[3]!=0.0) || (obs->P[0]!=0.0&&obs->P[1]!=0.0)) {
			if (satsys(obs->sat,NULL) == SYS_CMP)
				 y[1]=C1*obs->P[0]+C2*obs->P[3]-r-dant_if;//≤‚ ‘
            else y[1]=C1*obs->P[0]+C2*obs->P[1]-r-dant_if;
        }
        freq[0]=1.0;
    }
    else {
        for (i=0;i<nf;i++) {
			if ((freq[i]=sat2freq(obs->sat,obs->code[p[i]],nav))==0.0) continue;
            //if ((freq[i]=sat2freq(obs->sat,obs->code[i],nav))==0.0) continue;
            
            /* check SNR mask */
			if (testsnr(base,p[i],azel[1],obs->SNR[p[i]]*SNR_UNIT,&opt->snrmask)) {
				//trace(1, "SNR=%3.1f\n", obs->SNR[p[i]]*SNR_UNIT);
                continue;
            }
          /*  if (testsnr(base,i,azel[1],obs->SNR[i]*SNR_UNIT,&opt->snrmask)) {
			...
			if (obs->L[p[i]]!=0.0) y[i   ]=obs->L[p[i]]*CLIGHT/freq[i]-r-dant[p[i]];
            if (obs->P[p[i]]!=0.0) y[i+nf]=obs->P[p[i]]               -r-dant[p[i]];
                 ...
                 int index, double *y, double *e, double *azel, double *freq)
{
    ...
    for (i=0;i<n;i++) {
        ...
        zhd=tropmodel(obs[0].time,pos,zazel,0.0);//»¯ÀπÀ˛ƒ™ƒ˛ƒ£–Õ
        r+=tropmapf(obs[i].time,pos,azel+i*2,NULL)*zhd;//”≥…‰ƒ£–Õnmf
    ...
    trace(4,"rb_rr_=%.3f %.3f %.3f\n",rr_[0],rr_[1],rr_[2]);
    trace(4,"rb_pos=%.9f %.9f %.3f\n",pos[0]*R2D,pos[1]*R2D,pos[2]);
    for (i=0;i<n;i++) {
		satno2id(obs[i].sat, id);//≤‚ ‘
        trace(4,"id=%-3s sat=%3d %13.3f %13.3f %13.3f %13.10f %6.1f %5.1f\n",
              id,obs[i].sat,rs[i*6],rs[1+i*6],rs[2+i*6],dts[i*2],azel[i*2]*R2D,
              azel[1+i*2]*R2D);
    }
                 ...
                 int ns, double *v, double *H, double *R, int *vflg)
{
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        for (i=-1,j=0;j<ns;j++) {
            ...
            if (!validobs(iu[j],ir[j],f,nf,y)) continue;//iuŒ™“∆∂Ø’æ£¨irŒ™ª˘◊º’æ
        ...
        for (j=0;j<ns;j++) {
			...
			if (!validobs(iu[j], ir[j], f, nf, y)) { 
				//satno2id(sat[j], id1);//≤‚ ‘
				//trace(1,"invalidsat: sat=%3d id=%-3s\n",sat[j],id1);//≤‚ ‘
				continue; 
			}            
            if (H) {
                Hi=H+nv*rtk->nx;
            ...
            if (opt->tropopt==TROPOPT_EST||opt->tropopt==TROPOPT_ESTG) {
                ...
                for (k=0;k<(opt->tropopt<TROPOPT_ESTG?1:3);k++) {
					...
					trace(5, "j=%d idx=%3d %3d Hi=%.4f %.4f\n", j, (IT(0, opt) + k),
					(IT(1, opt) + k), Hi[IT(0, opt) + k], Hi[IT(1, opt) + k]);
					//≤‚ ‘
                }
            }
            /* DD phase-bias term */
            if (f<nf) {
                if (opt->ionoopt!=IONOOPT_IFLC) {
					...
					//trace(1, "dd=%12.3f\n", x[IB(sat[i], f, opt)] - x[IB(sat[j], f, opt)]);
                    if (H) {
                        Hi[IB(sat[i],f,opt)]= CLIGHT/freqi;
                        Hi[IB(sat[j],f,opt)]=-CLIGHT/freqj;
						trace(5, "j=%d idx=%3d %3d Hi=%.4f %.4f\n", j, IB(sat[i], f, opt),
						IB(sat[j], f, opt),Hi[IB(sat[i], f, opt)], Hi[IB(sat[j], f, opt)]);
						//≤‚ ‘
                    }
                }
                else {
			...
			if (f<nf) rtk->ssat[sat[j]-1].resc[p[f]]=v[nv];
			//{
			//	if (fabs(v[nv]) > 0.05)
			//	{
			//		rtk->ssat[sat[j] - 1].lock[f] = -1;//≤‚ ‘»Ù≤–≤Óπ˝¥Û£¨‘Ú‘› ±≤ª π”√¥Àø≈Œ¿–«
			//		//trace(1, "sat=%3d lock=%3d\n", sat[j] - 1,rtk->ssat[sat[j] - 1].lock[f]);
			//	}
			//	else rtk->ssat[sat[j] - 1].resc[f] = v[nv];
			//}
            else      rtk->ssat[sat[j]-1].resp[p[f-nf]]=v[nv];
            
            /* test innovation */
            if (opt->maxinno>0.0&&fabs(v[nv])>opt->maxinno) {
                if (f<nf) {
                    rtk->ssat[sat[i]-1].rejc[p[f]]++;
                    rtk->ssat[sat[j]-1].rejc[p[f]]++;
                }
				trace(1,"rejected (sat=%3d-%3d id=%-3s-%-3s %s%d v=%.3f)\n",
                       sat[i],sat[j],id1,id2,f<nf?"L":"P",f%nf+1,v[nv]);//≤‚ ‘
			...
			trace(4, "nv=%d Ri Rj=%.6f %.6f\n",nv,Ri[nv],Rj[nv]); //≤‚ ‘
            /* set valid data flags */
            if (opt->mode>PMODE_DGPS) {
                if (f<nf) rtk->ssat[sat[i]-1].vsat[p[f]]=rtk->ssat[sat[j]-1].vsat[p[f]]=1;
            }
            else {
                rtk->ssat[sat[i]-1].vsat[p[f-nf]]=rtk->ssat[sat[j]-1].vsat[p[f-nf]]=1;
            }
			//if (f < nf)
			//{
			//	satno2id(sat[i], id1);//≤‚ ‘
			//	satno2id(sat[j], id2);//≤‚ ‘
			//	trace(1,"sat=%3d id=%-3s sat=%3d id=%-3s vsat=%d\n",sat[i],id1,sat[j],id2,rtk->ssat[sat[j]-1].vsat[p[f]]);
			//}
   //         trace(1,"sat=%3d-%3d id=%-3s-%-3s %s%d v=%13.3f R=%8.6f %8.6f\n",sat[i],
   //               sat[j],id1,id2,f<nf?"L":"P",f%nf+1,v[nv],Ri[nv],Rj[nv]);
        ...
        for (f=0,k=na;f<nf;f++,k+=MAXSAT) {            
            for (i=k;i<k+MAXSAT;i++) {
                if (rtk->x[i]==0.0||!test_sys(rtk->ssat[i-k].sys,m)||
                    !rtk->ssat[i-k].vsat[p[f]]||!rtk->ssat[i-k].half[p[f]]) {
                    continue;
                }
                if (rtk->ssat[i-k].lock[p[f]]>0&&!(rtk->ssat[i-k].slip[p[f]]&2)&&
                    rtk->ssat[i-k].azel[1]>=rtk->opt.elmaskar&&!nofix) {
                    rtk->ssat[i-k].fix[p[f]]=2; /* fix */
					satno2id(i-na+1-f*MAXSAT, id_i);//≤‚ ‘
                    break;
                }
                else rtk->ssat[i-k].fix[p[f]]=1;
            }
			 //for (count=0,l=k,i=k;l<k+MAXSAT;l++) {
    //              if (rtk->x[l]==0.0||!test_sys(rtk->ssat[l-k].sys,m)||
    //                  !rtk->ssat[l-k].vsat[p[f]]||!rtk->ssat[l-k].half[p[f]]) {
    //                  continue;
    //              }
    //              if (rtk->ssat[l-k].lock[p[f]]>0&&!(rtk->ssat[l-k].slip[p[f]]&2)&&
    //                  rtk->ssat[l-k].azel[1]>=rtk->opt.elmaskar&&!nofix) {
				//	  if (l == k || rtk->ssat[l-k].azel[1] >= rtk->ssat[i-k].azel[1]) {
				//		  i = l;
    //                      rtk->ssat[i-k].fix[p[f]]=2; /* fix */
				//		  satno2id(i-na+1-f*MAXSAT, id_i);//≤‚ ‘
				//		  count++;
				//	  } 					
    //              }
    //              else rtk->ssat[l-k].fix[p[f]]=1;
    //         }
			//if(count > 0) trace(1, "ddidx: refsat=%3s\n", id_i);
            for (j=k;j<k+MAXSAT;j++) {
                if (i==j||rtk->x[j]==0.0||!test_sys(rtk->ssat[j-k].sys,m)||
                    !rtk->ssat[j-k].vsat[p[f]]) {
                    continue;
                }
                if (rtk->ssat[j-k].lock[p[f]]>0&&!(rtk->ssat[j-k].slip[p[f]]&2)&&
                    rtk->ssat[i-k].vsat[p[f]]&&
                    rtk->ssat[j-k].azel[1]>=rtk->opt.elmaskar&&!nofix) {
                    ix[nb*2  ]=i; /* state index of ref bias */
                    ix[nb*2+1]=j; /* state index of target bias */
                    nb++;
                    rtk->ssat[j-k].fix[p[f]]=2; /* fix */
					//satno2id(j-na+1-f*MAXSAT, id_j);//≤‚ ‘
					//trace(1, "fre=%d ref:id=%-3s ix=%3d  target:id=%-3s ix=%3d\n",
					//	f,id_i,i-2,id_j,j-2);  //≤‚ ‘
                }
                else rtk->ssat[j-k].fix[p[f]]=1;
            }
        }
    }
    return nb;
...
static void restamb(rtk_t *rtk, const double *bias, int nb, double *xa)
{
	...
	//if (rtk->x) { trace(1, "rtk->x=\n"); tracemat(1, rtk->x,rtk->nx, 1, 12, 4); }//≤‚ ‘
    
    for (m=0;m<5;m++) for (f=0;f<nf;f++) {
        
        for (n=i=0;i<MAXSAT;i++) {
            if (!test_sys(rtk->ssat[i].sys,m)||rtk->ssat[i].fix[p[f]]!=2) {
                continue;
            }
		...
		//for (k = i = 0; i < n; i++) {
		//	if (i == 0 || rtk->ssat[idx[i]].azel[1] >= rtk->ssat[idx[k]].azel[1]) {
		//		k = i;
		//	}
		//}
		//xa[index[k]]=rtk->x[index[k]];
  //      satno2id(idx[k]+1, id);//≤‚ ‘
		//trace(1, "restamb:refsat=%3s\n", id);
	  //  for (i=0;i<n;i++) {
			//if (i == k) continue;
   //          xa[index[i]]=xa[index[k]]-bias[nv++];
			////trace(1, "refindex=%3d xa=%12.4f index=%3d xa=%12.4f rtk->x=%12.4f dd=%12.4f\n",index[k],xa[index[k]],index[i],xa[index[i]],rtk->x[index[i]],bias[nv-1]);//≤‚ ‘
   //     }
        for (i=1;i<n;i++) {
            xa[index[i]]=xa[index[0]]-bias[nv++];
			////trace(1, "refindex=%3d xa=%12.4f index=%3d xa=%12.4f rtk->x=%12.4f dd=%12.4f\n",index[0],xa[index[0]],index[i],xa[index[i]],rtk->x[index[i]],bias[nv-1]);//≤‚ ‘
        }
    }
}
...
static void holdamb(rtk_t *rtk, const double *xa)
{
    ...
    if (nv>0) {
		...
		if (rtk->x) { trace(5, "update states with constraints£∫rtk-x=\n"); tracemat(5, rtk->x, rtk->nx, 1, 7, 4); }//≤‚ ‘
        free(R);
    }
    free(v); free(H);
}
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
    ...
    double *DP,*y,*b,*db,*Qb,*Qab,*QQ,s[2],*Qn,*Qbb;
    int *ix;
	...
	if (!(info = lambda(nb, 2, y, Qb, b, s))) {
		...
		if (s[0] <= 0.0 || s[1] / s[0] >= opt->thresar[0]) {
			...
			if (rtk->xa) { trace(5, "rtk->xa=\n"); tracemat(5, rtk->xa, 3, 1, 7, 4); }//≤‚ ‘
			for (i = 0; i < nb; i++) {
				bias[i] = b[i];
				y[i] -= b[i];
			}
                  ...
                  const nav_t *nav)
{
	...
	//trace(4, "P=\n"); tracemat(4, rtk->P, rtk->nx, rtk->nx, 13, 6);//≤‚ ‘
    trace(4,"x(0)="); tracemat(4,rtk->x,NR(opt),1,13,4);
    
    xp=mat(rtk->nx,1); Pp=zeros(rtk->nx,rtk->nx); xa=mat(rtk->nx,1);
    matcpy(xp,rtk->x,rtk->nx,1);
	if (rtk->x) { trace(5, "rtk->x=\n"); tracemat(5, rtk->x, 3, 1, 7, 4); }//≤‚ ‘
    ...
    for (i=0;i<niter;i++) {
        ...
        trace(4,"x(%d)=",i+1); tracemat(4,xp,rtk->nx,1,13,4);
    }
    if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,var,svh,nav,xp,opt,0,y,e,azel,
        ...
        if (valpos(rtk,v,R,vflg,nv,4.0)) {
			...
			if (rtk->x) { trace(5, "rtk->x=\n"); tracemat(5, rtk->x, rtk->nx, 1, 7, 4); }//≤‚ ‘
			if (rtk->P) { trace(5, "rtk->P=\n"); tracemat(5, rtk->P, rtk->nx, rtk->nx, 7, 4); }//≤‚ ‘
            
            /* update ambiguity control struct */
            rtk->sol.ns=0;
            for (i=0;i<ns;i++) for (f=0;f<nf;f++) {
                if (!rtk->ssat[sat[i]-1].vsat[p[f]]) continue;
                rtk->ssat[sat[i]-1].lock[p[f]]++;
                rtk->ssat[sat[i]-1].outc[p[f]]=0;
				satno2id(sat[i], id);//≤‚ ‘
				//trace(1,"valid satellite: f=%3d sat=%3d id=%-3s lock=%2d\n",p[f],sat[i],id,rtk->ssat[sat[i]-1].lock[p[f]]);//≤‚ ‘
    ...
    if (stat==SOLQ_FIX) {
        ...
        rtk->sol.qr[3]=(float)rtk->Pa[1];//xy
        rtk->sol.qr[4]=(float)rtk->Pa[1+2*rtk->na];//yz
        rtk->sol.qr[5]=(float)rtk->Pa[2];//xz
		trace(5, "rtk->sol.rr=\n"); tracemat(5, rtk->sol.rr, 3, 1, 7, 7); //≤‚ ‘
		trace(4, "sol=%.6f %.6f %.6f %.6f %.6f %.6f\n", rtk->sol.qr[0], rtk->sol.qr[1],
			rtk->sol.qr[2], rtk->sol.qr[3], rtk->sol.qr[4], rtk->sol.qr[5]); //≤‚ ‘
        ...
        if (rtk->opt.dynamics) { /* velocity and covariance */
			...
			trace(5, "rtk->sol.rr=\n"); tracemat(5, rtk->sol.rr, 6, 1, 7, 7); //≤‚ ‘
			trace(4, "sol.qv=%.6f %.6f %.6f %.6f %.6f %.6f\n", rtk->sol.qv[0], rtk->sol.qv[1],
				rtk->sol.qv[2], rtk->sol.qv[3], rtk->sol.qv[4], rtk->sol.qv[5]); //≤‚ ‘
        }
        rtk->nfix=0;
    }
    for (i=0;i<n;i++) for (j=0;j<nf;j++) {
        if (obs[i].L[p[j]]==0.0) continue;
        rtk->ssat[obs[i].sat-1].pt[obs[i].rcv-1][p[j]]=obs[i].time;
        rtk->ssat[obs[i].sat-1].ph[obs[i].rcv-1][p[j]]=obs[i].L[p[j]];
		/*trace(5, "i=%d f=%d rtk->ssat.pt=%s rtk->ssat.ph=%.10f\n",
			i,j, time_str(obs[i].time,3),obs[i].L[j]);*/  //≤‚ ‘
    }
    for (i=0;i<ns;i++) for (j=0;j<nf;j++) {
        
        /* output snr of rover receiver */
        rtk->ssat[sat[i]-1].snr[p[j]]=obs[iu[i]].SNR[p[j]];
    }
    for (i=0;i<MAXSAT;i++) for (j=0;j<nf;j++) {
        if (rtk->ssat[i].fix[p[j]]==2&&stat!=SOLQ_FIX) rtk->ssat[i].fix[p[j]]=1;
        if (rtk->ssat[i].slip[p[j]]&1) rtk->ssat[i].slipc[p[j]]++;
    }
...
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt)
{
	...
	rtk->nx = opt->mode <= PMODE_FIXED ? NX(opt) : NULL;
	rtk->na = opt->mode <= PMODE_FIXED ? NR(opt) : NULL;
    rtk->tt=0.0;
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    trace(1,"rtkpos  : time=%s n=%d\n",time_str(obs[0].time,3),n);
    trace(4,"obs=\n"); 
	trace(3, " index time id rcv L[0] L[1] P[0] P[1] LLI[0] LLI[1] code[0] code[1] SNR[0] SNR[1]\n");
	traceobs(4,obs,n);
