...
static double gettgd_BDS(int sat, const nav_t *nav, int type, int sign)
{
    int i,sys=satsys(sat,NULL);
	int k=pow(2.0,sign);
    ...
    if (sys==SYS_GLO) {
        ...
        for (i=0;i<nav->n;i++) {
            if (nav->eph[i].sat==sat && (nav->eph[i].DataFrom & k)) break;
        }
        return (i>=nav->n)?0.0:nav->eph[i].tgd[type]*CLIGHT;
    }
}
...
static int snrmask(const obsd_t *obs, const double *azel, const prcopt_t *opt)
{
	...
	if (opt->ionoopt != IONOOPT_IFLC) {
		if (testsnr(0,p[0],azel[1],obs->SNR[p[0]]*SNR_UNIT,&opt->snrmask)) return 0;       
    }
    else if (opt->ionoopt==IONOOPT_IFLC && satsys(obs->sat,NULL)==SYS_CMP) {
         if (testsnr(0,3,azel[1],obs->SNR[3]*SNR_UNIT,&opt->snrmask)) return 0;
    }
	else if (opt->ionoopt==IONOOPT_IFLC){
		 if (testsnr(0,0,azel[1],obs->SNR[0]*SNR_UNIT,&opt->snrmask) && 
			 testsnr(0,1,azel[1],obs->SNR[1]*SNR_UNIT,&opt->snrmask)) return 0;
	}
                     ...
                     double *var)
{
    ...
    int sat,sys;
	int *p = opt->fre;
	double freq_BDS1,freq_BDS2;

    sat=obs->sat;

    sys=satsys(sat,NULL);
	if(sys == SYS_CMP && opt->ionoopt==IONOOPT_IFLC)
	{
	    code2freq_BDS(obs->code[p[0]], &freq_BDS1);
	    code2freq_BDS(obs->code[p[1]], &freq_BDS2);
	}
    P1=obs->P[p[0]];
	P2=obs->P[p[1]];
    *var=0.0;
    ...
    if ((sys==SYS_GPS||sys==SYS_GLO) && 0 == p[0]) {
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* dual-frequency */
        ...
        if (sys==SYS_GPS||sys==SYS_QZS) { /* L1-L2,G1-G2 */
			...
			//gamma=SQR(((obs->code[0]==CODE_L2I)?FREQ1_CMP:FREQ1)/FREQ3_CMP);
   ...
   //         b2=gettgd(sat,nav,3); /* TGD_B2a (m) */
   //         return ((P2-gamma*P1)+gamma*b1)/(1.0-gamma);

            gamma=SQR(freq_BDS1/freq_BDS2);
            if      (obs->code[p[0]]==CODE_L2I) b1=gettgd(sat,nav,0); /* TGD_B1I */
            else if (obs->code[p[0]]==CODE_L1P || obs->code[p[0]]==CODE_L1X) b1=gettgd_BDS(sat,nav,2,p[0]); /* TGD_B1Cp */
			else if (obs->code[p[0]]==CODE_L5P || obs->code[p[0]]==CODE_L5X) b1=gettgd_BDS(sat,nav,3,p[0]); /* TGD_B2ap */

			if      (obs->code[p[1]]==CODE_L2I) b2=gettgd(sat,nav,0); /* TGD_B1I */
			else if (obs->code[p[1]]==CODE_L6I) b2=0;                 /* TGD_B3I */
            else if (obs->code[p[1]]==CODE_L1P || obs->code[p[1]]==CODE_L1X) b2=gettgd_BDS(sat,nav,2,p[1]); /* TGD_B1Cp */
			else if (obs->code[p[1]]==CODE_L5P || obs->code[p[1]]==CODE_L5X) b2=gettgd_BDS(sat,nav,3,p[1]); /* TGD_B2ap */
        ...
        *var=SQR(ERR_CBIAS);
        ...
        if (sys==SYS_GPS||sys==SYS_QZS) { /* L1 */
            ...
            if      (obs->code[p[0]]==CODE_L2I) b1=gettgd(sat,nav,0); /* TGD_B1I */
			else if (obs->code[p[0]]==CODE_L2I) b1=gettgd(sat,nav,1); /* TGD_B1I */
            else if (obs->code[p[0]]==CODE_L1P || obs->code[p[0]]==CODE_L1X) b1=gettgd_BDS(sat,nav,2,p[0]); /* TGD_B1Cp */
			else if (obs->code[p[0]]==CODE_L5P || obs->code[p[0]]==CODE_L5X) b1=gettgd_BDS(sat,nav,3,p[0]); /* TGD_B2ap */
            else b1=gettgd(sat,nav,2)+gettgd(sat,nav,4); /* TGD_B1Cp+ISC_B1Cd */
            return P1-b1;
                    ...
                    const double *azel, int ionoopt, double *ion, double *var)
{
    ...
    trace(4,"ionocorr: time=%s opt=%d id=%-3s sat=%2d pos=%.3f %.3f azel=%.3f %.3f\n",
          time_str(time,3),ionoopt,id,sat,pos[0]*R2D,pos[1]*R2D,azel[0]*R2D,
          azel[1]*R2D);
    ...
    if (ionoopt==IONOOPT_BRDC) {
		//if (sys == SYS_CMP) {
		//	int k = floor(ep[3]);
		//	if (norm(nav->ion_bds + k, 8)) {
		//	   *ion=ionmodel_BDS(time,nav->ion_bds+k,pos,azel);
  //             *var=SQR(*ion*ERR_BRDCI);
  //             return 1;
		//	}
		//	*ion=ionmodel_BDS(time,nav->ion_cmp,pos,azel);
  //          *var=SQR(*ion*ERR_BRDCI);
  //          return 1;
		//}
...
static int rescode(sol_t *sol, int iter, const obsd_t *obs, int n, const double *rs,
                   const double *dts, const double *vare, const int *svh,
                   const nav_t *nav, const double *x, const prcopt_t *opt,
                   double *v, double *H, double *var, double *azel, int *vsat,
                   double *resp, double *ion, int *ns)
{
    gtime_t time;
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
		...
		if (iter == 0) {
			...
			if (sys == SYS_GPS) sol->satnum[0]++;
				...
				if (sat <= 121 && sat >= 106) sol->satnum[3]++;
				else sol->satnum[4]++;
			}
		}
        ...
        /* excluded satellite? */
        if (satexclude(sat,vare[i],svh[i],opt)) continue;//剔除手动排除的卫星与未参与解算系统的全部卫星
        ...
        if (iter>0) {
            ...
            if ((freq=sat2freq(sat,obs[i].code[p[0]],nav))==0.0) continue;
            dion*=SQR(FREQ1/freq);//电离层改正模型均是基于L1频率，利用电离层延迟与频率的关系将电离层延迟变换到当前频率下
            vion*=SQR(FREQ1/freq);
        ...
        /* design matrix */
        for (j=0;j<NX;j++) {
            H[j+nv*NX]=j<3?-e[j]:(j==3?1.0:0.0);//对H逐行赋值
        }
        ...
        vsat[i]=1; resp[i]=v[nv]; ion[i]=dion;(*ns)++;
        
        /* variance of pseudorange error */
        var[nv++]=varerr(opt,azel[1+i*2],sys)+vare[i]+vmeas+vion+vtrp;
        
		satno2id(obs[i].sat, id);//测试
        trace(4,"idx=%2d id=%-3s sat=%2d azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n",
			nv,id,obs[i].sat,azel[i*2]*R2D,azel[1+i*2]*R2D,resp[i],sqrt(var[nv-1]));
    }
...
/* validate solution ---------------------------------------------------------*/
static int valsol(sol_t *sol, const double *azel, const int *vsat, int n,
                  ...
                  char *msg)
{
    ...
    vv=dot(v,v,nv);
	//trace(1,"v="); tracemat(1,v,nv,1,13,4);
...
static int estpos(const obsd_t *obs, int n, const double *rs, const double *dts,
                  ...
                  double *resp, double *ion,char *msg)
{
	...
	for (i=0;i<5;i++) sol->satnum[i]=0;
    for (i=0;i<3;i++) x[i]=sol->rr[i];
    
    for (i=0;i<MAXITR;i++) {
        
        /* pseudorange residuals (m) */
        nv=rescode(sol,i,obs,n,rs,dts,vare,svh,nav,x,opt,v,H,var,azel,vsat,resp,ion,
                   &ns);//i为迭代次数，ns为有效卫星数，注意第一次迭代不进行卫星有效性检测
        ...
        if (norm(dx,NX)<1E-4) {
            ...
            /* validate solution */
            if ((stat=valsol(sol,azel,vsat,n,opt,v,nv,NX,msg))) {
...
{
    prcopt_t opt_=*opt;
    double *rs,*dts,*var,*azel_,*resp,*ion;
    ...
    stat=estpos(obs,n,rs,dts,var,svh,nav,&opt_,sol,azel_,vsat,resp,ion,msg);
