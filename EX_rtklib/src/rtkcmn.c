...
const prcopt_t prcopt_default={ /* defaults processing options */
	PMODE_SINGLE,0,2,{0,2,4},SYS_GPS,   /* mode,soltype,nf,fre,navsys */
									 /*GPS: 0-L1;1-L2;2-L5
									   GLONASS: 0-G1/G1a;1-G2/G2a;2-G3
                                       Galileo: 0-E1;1-E5b;2-E5a;3-E6;4-E5a+E5b;
                                       QZSS: 0-L1;1-L2;2-L5;3-LEX;
                                       BeiDou: 0-B1I/B1C;1-B2I/B2b;2-B2a;3-B3I;4-B2ab*/
	//15.0*D2R,{{1,1},{{40,40,40,40,40,40,40,40,40},{40,40,40,40,40,40,40,40,40},
 //   {40,40,40,40,40,40,40,40,40},{40,40,40,40,40,40,40,40,40},{40,40,40,40,40,40,40,40,40}}}, /* elmin,snrmask */ //测试
    ...
    0,0,                        /* rovpos,refpos */
    {100.0,100.0,100.0,100.0,100.0},   /* eratio[] */
	...
	0,0,0,{"-SYS=CG","-SYS=CG"},   /* maxaveep,initrst,outsingle,rnxopt*/ //测试
	{1,1}                       /*posopt*/
};
...
extern uint8_t obs2code(const char *obs)
{
    ...
    for (i=1;*obscodes[i];i++) {
        if (strcmp(obscodes[i],obs)) continue;//strcmp输入为地址
        return (uint8_t)i;
    }
    return CODE_NONE;
}
...
extern int getcodepri(int sys, uint8_t code, const char *opt)
{
    ...
    for (p=opt;p&&(p=strchr(p,'-'));p++) {
        if (sscanf(p,optstr,str)<1||str[0]!=obs[0]) continue;//从字符串中读取格式化数据
...
extern uint32_t getbitu(const uint8_t *buff, int pos, int len)
{
    ...
    for (i=pos;i<pos+len;i++) 
	{
	    ...
	    //trace(4,bits="%d",buff[i/8]>>(7-i%8));//测试
	}
    return bits;
}
                   ...
                   double *xp, double *Pp)
{
    ...
    if (!(info=matinv(Q,m))) {
		//if (Q) { trace(5, "Q=\n"); tracemat(5, Q, m, m, 13, 6); }//测试
        matmul("NN",n,m,m,1.0,F,Q,0.0,K);   /* K=P*H*Q^-1 */
        matmul("NN",n,1,m,1.0,K,v,1.0,xp);  /* xp=x+K*v */
        matmul("NT",n,n,m,-1.0,K,H,1.0,I);  /* Pp=(I-K*H')*P */
		//if (I) { trace(5, "I=\n"); tracemat(5, I, n, n, 13, 6); }//测试
        matmul("NN",n,n,n,1.0,I,P,0.0,Pp);
		//if (Pp) { trace(5, "Pp=\n"); tracemat(5, Pp, n, n, 13, 6); }//测试
    }
                  ...
                  const double *R, int n, int m)
{
	...
	//if (P_) { trace(5, "P_=\n"); tracemat(5, P_, k, k, 7, 4); }//测试
	//if (H_) { trace(5, "H_=\n"); tracemat(5, H_, k, m, 7, 4); }//测试
    info=filter_(x_,P_,H_,v,R,k,m,xp_,Pp_);
	//if (Pp_) { trace(5, "Pp_=\n"); tracemat(5, Pp_, k, k, 7, 4); }//测试
    for (i=0;i<k;i++) {
        x[ix[i]]=xp_[i];
        for (j=0;j<k;j++) P[ix[i]+ix[j]*n]=Pp_[i+j*k];
    }
	//if (P) { trace(5, "P=\n"); tracemat(5, P, n, n, 7, 4); }//测试
...
extern gtime_t epoch2time(const double *ep)
{
    ...
    days=(year-1970)*365+(year-1969)/4+doy[mon-1]+day-2+(year%4==0&&mon>=3?1:0);//1968年为闰年
    sec=(int)floor(ep[5]);
...
{
    gtime_t t0=epoch2time(bdt0);//将BDS时初始时刻(2006.1.1)化为g_time时
    time_t sec=t.time-t0.time;
    int w=(int)(sec/(86400*7));
...
/* eci(地心惯性坐标系) to ecef transformation matrix -------------------------------------------
...
static void uniqeph(nav_t *nav)
{
    ...
    for (i=1,j=0;i<nav->n;i++) {
		satno2id(nav->eph[i].sat, id);//测试
		trace(3, "sortsnavdata: ttr=%s ", time_str(nav->eph[i].ttr, 3));
		trace(3, "toe=%s id=%-3s sat=%-3d\n",
			time_str(nav->eph[i].toe, 3), id,nav->eph[i].sat);//测试,一个括号内使用两个time_str会出错
...
static void uniqgeph(nav_t *nav)
{
    ...
    for (i=j=0;i<nav->ng;i++) {
		satno2id(nav->geph[i].sat, id);//测试
		trace(3, "sortsnavdata: tof=%s ", time_str(nav->geph[i].tof, 3));
		trace(3, "toe=%s id=%-3s sat=%-3d\n",
			time_str(nav->geph[i].toe, 3), id,nav->geph[i].sat);//测试,一个括号内使用两个time_str会出错
...
static void uniqseph(nav_t *nav)
{
    ...
    for (i=j=0;i<nav->ns;i++) {
		satno2id(nav->seph[i].sat, id);//测试
		trace(3, "sortsnavdata: tof=%s ", time_str(nav->seph[i].tof, 3));
		trace(3, "t0=%s id=%-3s sat=%-3d\n",
			time_str(nav->seph[i].t0, 3), id,nav->seph[i].sat);//测试,一个括号内使用两个time_str会出错
...
extern int sortobs(obs_t *obs)
{
    ...
    for (i=j=0;i<obs->n;i++) {
		satno2id(obs->data[i].sat, id);//测试
		trace(3, "sortsobsdata:time=%s rcv=%-1d id=%-3s sat=%-3d L=%-20.14f\n",
			time_str(obs->data[i].time, 3),
			obs->data[i].rcv,id,obs->data[i].sat, obs->data[i].L[0]);//测试
...
*-----------------------------------------------------------------------------*/
extern double ionmodel_BDS(gtime_t t, const double *ion, const double *pos,
                       const double *azel)
{
    const double ion_default[]={ /* 2017/8/23 */
        1.3039E-08, 4.3958E-07,-3.4571E-06, 6.4969E-06,
        1.3517E+05,-5.4067E+05, 3.0147E+06,-2.7525E+06
    };
	//计算电离层穿刺点IPP的地理纬度psi与地理经度lam
	double cosaz,rp,ap,sinap,tanap,re=6378E3,hion=375E3,psi,lam;//re为地球半径,hion为电离层单层高度   
    rp=re/(re+hion)*cos(azel[1]);
    ap=PI/2.0-azel[1]-asin(rp);
    sinap=sin(ap);
    tanap=tan(ap);
    cosaz=cos(azel[0]);
    psi=asin(sin(pos[0])*cos(ap)+cos(pos[0])*sinap*cosaz);
    
    if ((pos[0]> 70.0*D2R&& tanap*cosaz>tan(PI/2.0-pos[0]))||
        (pos[0]<-70.0*D2R&&-tanap*cosaz>tan(PI/2.0+pos[0]))) {
        lam=pos[1]+PI-asin(sinap*sin(azel[0])/cos(psi));
    }
    else {
        lam=pos[1]+asin(sinap*sin(azel[0])/cos(psi));
    }
    double tt,amp,per,x;
    int week;
    ...
    /* local time (s) */
    tt=43200.0*lam/PI+time2bdt(t,&week);
    tt-=floor(tt/86400.0)*86400.0; /* 0<=tt<86400 */
        
    /* ionospheric delay */
    amp=ion[0]+(psi/PI)*(ion[1]+(psi/PI)*(ion[2]+(psi/PI)*ion[3]));
    per=ion[4]+(psi/PI)*(ion[5]+(psi/PI)*(ion[6]+(psi/PI)*ion[7]));
    amp=amp<    0.0?    0.0:amp;
    per=per>172800?172800:(per<72000?72000:per);
    x=2.0*PI*(tt-50400.0)/per;
    
    return CLIGHT*(fabs(x)<1.57?5E-9+amp*x:5E-9);
}
...
extern void excludesats(char *exsats,prcopt_t *popt) 
{
	char buff[1024], *p, *id;
	int sat;
	if (exsats[0] != '\0') {
