                    ...
                    double *var)
{
    ...
    #define Aref1   27906100
    #define Aref2   42162200
	double A0, Ak, delNA;
	if (sys==SYS_CMP && eph->DataFrom&7)
	{
	   if (eph->SatType == 1 || eph->SatType == 2)
	   {
		   A0=Aref2+eph->delA;
		   Ak=A0+eph->Adot*tk;
		   delNA = eph->deln + 0.5*eph->delndot*tk;
		   M=eph->M0+(sqrt(mu/(A0*A0*A0))+delNA)*tk;
	   }
	   else
	   {
		   A0=Aref1+eph->delA;
	...
	else M=eph->M0+(sqrt(mu/(eph->A*eph->A*eph->A))+eph->deln)*tk;
	...
	if (sys==SYS_CMP && eph->DataFrom&7) r=Ak*(1.0-eph->e*cosE);
    else r=eph->A*(1.0-eph->e*cosE);
                  ...
                  int *svh)
{
	char id[64];//≤‚ ‘
	satno2id(sat, id);//≤‚ ‘
    trace(4,"satpos  : time=%s id=%-3s sat=%2d ephopt=%d\n",time_str(time,3),id,sat,ephopt);
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        /*trace(1,"%s id=%-3s sat=%3d rs=%13.3f %13.3f %13.3f %13.3f %13.3f %13.3f dts=%12.3f %12.3f var=%7.3f svh=%02d\n",
              time_str(time[i],6),id,obs[i].sat,rs[i*6],rs[1+i*6],rs[2+i*6],
			  rs[3+i*6],rs[4+i*6],rs[5+i*6],dts[i*2]*1E9,
              dts[1+i*2]*1E9,var[i],svh[i]);*/
    }
}
